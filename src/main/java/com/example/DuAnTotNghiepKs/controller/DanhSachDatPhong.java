...
public class DanhSachDatPhong {
    ...
    @Autowired
    private KhachHangDD khachHangDD;
    ...
    public ResponseEntity<?> checkInDatPhong(@RequestParam List<Integer> idDatPhongs) {
        try {
            // Map để lưu danh sách đặt phòng theo khách hàng
            Map<KhachHang, List<DatPhong>> datPhongsByKhachHang = new HashMap<>();
            for (Integer idDatPhong : idDatPhongs) {
                ...
                // Nhóm đặt phòng theo khách hàng
                KhachHang khachHang = datPhong.getKhachHang();
                datPhongsByKhachHang
                        .computeIfAbsent(khachHang, k -> new ArrayList<>())
                        .add(datPhong);
            }
            // Gửi email cho từng khách hàng
            datPhongsByKhachHang.forEach((khachHang, datPhongs) ->{
    ...
    public ResponseEntity<?> confirmCheckIn(@RequestParam List<Integer> idDatPhongs) {
        try {
            // Map để lưu danh sách đặt phòng theo khách hàng
            Map<KhachHang, List<DatPhong>> datPhongsByKhachHang = new HashMap<>();
            ...
            // Gửi email cho từng khách hàng
            for (Map.Entry<KhachHang, List<DatPhong>> entry : datPhongsByKhachHang.entrySet()) {
                KhachHang khachHang = entry.getKey();
                List<DatPhong> datPhongs = entry.getValue();

                // Tạo nội dung email
                StringBuilder emailContent = new StringBuilder();
                emailContent.append("<html>")
                        .append("<body style='font-family: Arial, sans-serif;'>")
                        .append("<h2 style='color: #4CAF50;'>Xin chào ").append(khachHang.getHoVaTen()).append(",</h2>")
                        .append("<p>Dưới đây là thông tin các đặt phòng đã check-in thành công:</p>")
                        .append("<table style='border-collapse: collapse; width: 100%;'>")
                        .append("<thead>")
                        .append("<tr>")
                        .append("<th style='border: 1px solid #ddd; padding: 8px;'>Phòng</th>")
                        .append("<th style='border: 1px solid #ddd; padding: 8px;'>Ngày nhận</th>")
                        .append("<th style='border: 1px solid #ddd; padding: 8px;'>Ngày trả</th>")
                        .append("<th style='border: 1px solid #ddd; padding: 8px;'>Nhân viên check-in</th>")
                        .append("</tr>")
                        .append("</thead>")
                        .append("<tbody>");
                ...
                for (DatPhong datPhong : datPhongs) {
                    emailContent.append("<tr>")
                            .append("<td style='border: 1px solid #ddd; padding: 8px;'>").append(datPhong.getPhong().getTenPhong()).append("</td>")
                            .append("<td style='border: 1px solid #ddd; padding: 8px;'>").append(datPhong.getNgayNhan()).append("</td>")
                            .append("<td style='border: 1px solid #ddd; padding: 8px;'>").append(datPhong.getNgayTra()).append("</td>")
                            .append("<td style='border: 1px solid #ddd; padding: 8px;'>").append(datPhong.getNhanVienCheckIn().getHoTen()).append("</td>")
                            .append("</tr>");
                }

                emailContent.append("</tbody></table>")
                        .append("<p>Chúng tôi hy vọng bạn sẽ có một kỳ nghỉ thoải mái!</p>")
                        .append("<p>Trân trọng,</p>")
                        .append("<p>Đội ngũ quản lý DRAGONBALL HOTEL</p>")
                        .append("</body></html>");

                // Gửi email
                String subject = "Xác nhận check-in tại DRAGONBALL HOTEL";
                datPhongService.sendEmail(khachHang.getEmail(), subject, emailContent.toString());
            }
